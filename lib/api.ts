import type { CollectionRef, Link, Profile, Related, Rel, Trail, TrailCollection } from './types'
...
export interface Api {
  ...
  getTrailCollection(handleOrDid: string, recordKey: string): Promise<TrailCollection>
}
...
const mockApi: Api = {
  ...
  async saveCollection(trail, profile) {
    ...
    return { uri, recordKey, handle: profile.handle, url: `/user/${profile.handle}/trail/${recordKey}` }
  },
...
const cardToLink = (c: UrlCard): Link => metadataToLink(c.cardContent ?? { url: c.url })
...
async function fetchMutual(url: string): Promise<Related[]> {
  const { collections } = await xrpc<{ collections: { id: string }[] }>(
    ...
    { params: { url, limit: MUTUAL_COLLECTIONS, sortBy: 'updatedAt', sortOrder: 'desc' } }
  )
...
async function fetchSimilar(url: string): Promise<Related[]> {
  const { urls } = await xrpc<{ urls: { url: string; metadata: UrlMetadata }[] }>(
...
async function fetchConnected(url: string): Promise<Related[]> {
  ...
  return connections
    .map((c) => {
      ...
      const note = [c.connection.type?.toLowerCase().replace(/_/g, ' '), c.connection.note]
        .filter(Boolean)
        .join(' — ')
      return { ...metadataToLink(other.metadata), rel: 'connected' as const, note: note || undefined }
    })
    .filter((r) => r.url !== url)
}
...
interface FeedActivity {
  card?: UrlCard
  connection?: { source: { url: string; metadata: UrlMetadata }; target: { url: string; metadata: UrlMetadata } }
}

async function getFeedPool(): Promise<Link[]> {
  if (feedPool) return feedPool
  const { activities } = await xrpc<{ activities: FeedActivity[] }>('network.cosmik.feed.getGlobal', {
...
const sembleApi: Api = {
  async getLink(url) {
    const { metadata } = await xrpc<{ metadata: UrlMetadata }>('network.cosmik.card.getUrlMetadata', {
      params: { url },
    })
  ...
  async getSeedResults(urls) {
    const perSeed = await Promise.all(urls.map((u) => sembleRelated(u).catch(() => [] as Related[])))
    return dedupeByUrl(perSeed.flat()).filter((r) => !urls.includes(r.url))
  },

  async getRandomLinks(n, excludeUrls = []) {
    const pool = (await getFeedPool()).filter((l) => !excludeUrls.includes(l.url))
    const out: Link[] = []
    for (let i = 0; i < n && pool.length; i++) {
      out.push(pool.splice(Math.floor(Math.random() * pool.length), 1)[0])
    }
  ...
  async saveCollection(trail, profile) {
    const { collectionId } = await xrpc<{ collectionId: string }>('network.cosmik.collection.create', {
      body: { name: trail.title, description: trail.description, accessType: 'OPEN' },
    })
    ...
    for (const link of dedupeByUrl([...trail.seeds, ...trail.path])) {
      await xrpc('network.cosmik.card.addUrl', { body: { url: link.url, collectionIds: [collectionId] } })
    }
  ...
  async signIn(apiKey) {
    ...
    return { handle: me.handle, displayName: me.name || me.handle, avatar: me.avatarUrl }
  },

  async getTrailCollection(handleOrDid, recordKey) {
    const page = await xrpc<CollectionPage>('network.cosmik.collection.getByAtUri', {
